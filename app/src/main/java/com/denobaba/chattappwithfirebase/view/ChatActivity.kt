package com.denobaba.chattappwithfirebase.view
...
import com.denobaba.chattappwithfirebase.adapter.messageadapter
import com.denobaba.chattappwithfirebase.databinding.ActivityChatBinding
import com.denobaba.chattappwithfirebase.dataclasses.messageclass
...
class ChatActivity : AppCompatActivity() {
    ...
    fun sent(view: View){
        ...
        firestore.collection("messages").add(postMap).addOnSuccessListener { //firestoreye verileri yazdırdık

                binding.yourmessage.setText("")
        }.addOnFailureListener {
            Toast.makeText(this,it.localizedMessage,Toast.LENGTH_LONG).show()  //failure olursa ne yaptık ona baktık
    ...
    private fun getData(){
        firestore.collection("messages").orderBy("date").addSnapshotListener { value, error ->          //collectionname firebasedatabasede bulabilirsin
            if (error !=null){
                ...
                if(value != null){
                    if(!value.isEmpty){  //değer varsa
                        ...
                        postArrayList.clear()
                        for (i in documents){
                            val yourmessage= i.get("yourmessage") as String
                            val userid = i.get("userid").toString()
                            val post = messageclass(yourmessage,userid)       //açtığımız data class a yazdırıyoruz
